...
export struct download_response {
...
export auto download(char const* url,
    std::optional<std::chrono::milliseconds> timeout = std::nullopt) -> std::expected<download_response, std::string> {
...
export auto download(std::string const& url, std::optional<std::chrono::milliseconds> timeout = std::nullopt) {
